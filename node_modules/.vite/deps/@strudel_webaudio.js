...
var Wt = (t, e = 3) => {
  ...
  const c = ln[n.toLowerCase()], s = (o == null ? void 0 : o.split("").reduce((d, l) => d + rn[l], 0)) || 0;
...
function Ve(t, e) {
  ...
  for (let p = 0; p < o; p++)
    if (t === "white")
      ...
      let Z2 = Math.random() * 2 - 1;
      d = 0.99886 * d + Z2 * 0.0555179, l = 0.99332 * l + Z2 * 0.0750759, i = 0.969 * i + Z2 * 0.153852, r = 0.8665 * r + Z2 * 0.3104856, h = 0.55 * h + Z2 * 0.5329522, m = -0.7616 * m - Z2 * 0.016898, c[p] = d + l + i + r + h + m + u + Z2 * 0.5362, c[p] *= 0.11, u = Z2 * 0.115926;
    } else if (t === "crackle") {
      const Z2 = e * 0.01;
...
var Q = (t, e, n, o, a, c, s, d, l, i = "exponential") => {
  ...
  const h = s - c, m = s, u = c + o * h, p = l - d, Z2 = (X) => {
    let y;
    if (e > X) {
      let b = me(c, m, 0, e);
      y = X * b + (c > m ? c : 0);
  ...
  t.setValueAtTime(c, d), e > p ? t[r](Z2(p), l) : e + n > p ? (t[r](Z2(e), d + e), t[r](Z2(p), l)) : (t[r](Z2(e), d + e), t[r](Z2(e + n), d + e + n), t.setValueAtTime(u, l)), t[r](c, l + a);
};
...
function St(t, e, n, o, a, c, s, d, l, i, r, h, m, u) {
  ...
  let V2, g;
  if (m === "ladder" ? (V2 = I(t, "ladder-processor", { frequency: n, q: o, drive: u }), g = V2.parameters.get("frequency")) : (V2 = t.createBiquadFilter(), V2.type = e, V2.Q.value = o, V2.frequency.value = n, g = V2.frequency), (a ?? c ?? s ?? d ?? l) !== void 0) {
    l = U(l, 1, true), h = U(h, 0, true);
    const M = Math.abs(l), w = M * h;
    let Y = $(2 ** -w * n, 0, 2e4), F2 = $(2 ** (M - w) * n, 0, 2e4);
    return l < 0 && ([Y, F2] = [F2, Y]), Q(g, Z2, X, y, b, Y, F2, i, r, p), V2;
  }
  return V2;
}
...
function Nt(t, e, n) {
  ...
  if (a) {
    const X = W2().createGain(), y = Vn(t, o, a, h);
    if (u = y.node, p = y.stop, ![s, d, l, i, r].some((b) => b !== void 0))
      u.connect(t);
    else {
      const [b, V2, g, f2] = B([s, d, l, i]), M = n + m;
      Q(
        X.gain,
        b,
        V2,
...
function Mn(t) {
  const e = W2(), n = e.createBuffer(t.numberOfChannels, t.length, e.sampleRate);
...
async function Yn(t, e, n, o, a) {
  ...
  i == null && l == null && e.release == null && (m = V2);
  let F2 = t + m;
...
var W2 = () => Gt || Nn();
function Xo() {
  return W2().currentTime;
}
var Ht;
function Cn() {
  return Ht || (Ht = W2().audioWorklet.addModule(pn)), Ht;
}
async function vn(t = {}) {
  const {
...
var nt;
function Pn() {
  const t = W2(), e = t.destination.maxChannelCount;
...
var Go = () => {
  nt != null && (nt.gain.linearRampToValueAtTime(0, W2().currentTime + 0.01), nt = null);
};
function kn(t, e, n, o) {
  var _a;
  if (n = $(n, 0, 0.98), !x[t].delayNode) {
    const c = W2().createFeedbackDelay(1, e, n);
...
function Qn(t, e, n = 1, o = 0.5, a = 1e3, c = 2e3) {
  const s = W2(), d = Qt(s, t, e, { frequency: n, depth: c * 2 }), l = 2;
  let i = 0;
  const r = [];
  for (let h = 0; h < l; h++) {
...
function On(t, e, n, o, a, c, s, d) {
  if (!x[t].reverbNode) {
    const i = W2().createReverb(e, n, o, a, c, s, d);
...
var En = async (t, e, n, o = 0.5, a = 0.5) => {
  ...
  Un(c, E, nn), V2 != null && jn(c, V2, e, g, f2), X = A(U(X, 1)), y = A(y), Mt = A(Mt), Rt = A(Rt), rt = A(rt), Lt = A(Lt), i = A(i), X *= Lt;
  ...
  if (Z2)
    ...
    const { onTrigger: G } = Tt(u), K = await G(e, t, () => {
      tt.forEach((N) => N == null ? void 0 : N.disconnect()), at.delete(se);
    });
...
function xo() {
  [...An].forEach((t) => {
    ...
    "sbd",
    (t, e, n) => {
      const { duration: o, decay: a = 0.5, pdecay: c = 0.5, penv: s = 36, clip: d } = e, l = W2(), i = 0.02, r = 1.2, h = 0.025, m = 1, u = l.createOscillator();
    ...
    (t, e, n) => {
      const o = [
        ...
        "((t^t/2+t+64)%7 * 24)"
      ], { n: a = 0 } = e, c = dt(e), { byteBeatExpression: s = o[a % o.length], byteBeatStartTime: d } = e, l = W2();
      let { duration: i } = e;
      const [r, h, m, u] = B(
        [e.attack, e.decay, e.sustain, e.release],
        "linear",
...
function _n(t, e) {
  const n = new Float32Array(t + 1), o = new Float32Array(t + 1), a = W2(), c = a.createOscillator(), s = {
    sawtooth: (i) => [0, -1 / i],
...
function to(t, e, n) {
  ...
  !o || t === "sine" ? (s = W2().createOscillator(), s.type = t || "triangle") : s = _n(o, t), s.frequency.value = dt(n), s.start(e);
  ...
  return c && (i = Zn(s, c, e)), {
    ...
    stop: (r) => {
      l.stop(r), d == null ? void 0 : d.stop(r), i == null ? void 0 : i.stop(r), s.stop(r);
    },
    triggerRelease: (r) => {
    }
  };
}
function eo(t = 1, e = 0.05, n = 220, o = 0, a = 0, c = 0.1, s = 0, d = 1, l = 0, i = 0, r = 0, h = 0, m = 0, u = 0, p = 0, Z2 = 0, X = 0, y = 1, b = 0, V2 = 0) {
  let g = Math.PI * 2, f2 = W2().sampleRate, M = (gt) => gt > 0 ? 1 : -1, w = l *= 500 * g / f2 / f2, Y = n *= (1 + e * 2 * Math.random() - e) * g / f2, F2 = [], S2 = 0, O2 = 0, z = 0, T2 = 1, k = 0, xt = 0, C2 = 0, _, D;
  for (o = o * f2 + 9, b *= f2, a *= f2, c *= f2, X *= f2, i *= 500 * g / f2 ** 3, p *= g / f2, r *= g / f2, h *= f2, m = m * f2 | 0, D = o + b + a + c + X | 0; z < D; F2[z++] = C2)
    ++xt % (Z2 * 100 | 0) || (C2 = s ? s > 1 ? s > 2 ? s > 3 ? Math.sin((S2 % g) ** 3) : Math.max(Math.min(Math.tan(S2), 1), -1) : 1 - (2 * S2 / g % 2 + 2) % 2 : 1 - 4 * Math.abs(Math.round(S2 / g) - S2 / g) : Math.sin(S2), C2 = (m ? 1 - V2 + V2 * Math.sin(g * z / m) : 1) * M(C2) * Math.abs(C2) ** d * // curve 0=square, 2=pointy
    t * 1 * // envelope
    (z < o ? z / o : z < o + b ? 1 - (z - o) / b * (1 - y) : z < o + b + a ? y : z < D - X ? (D - z - X) / c * // release falloff
    y : 0), C2 = X ? C2 / 2 + (X > z ? 0 : (z < D - X ? 1 : (D - z) / X) * // release delay
    F2[z - X | 0] / 2) : C2), _ = (n += l += i) * // frequency
    Math.cos(p * O2++), S2 += _ - _ * u * (1 - (Math.sin(z) + 1) * 1e9 % 2), T2 && ++T2 > h && (n += r, Y += r, T2 = 0), m && !(++k % m) && (n = Y, l = w, T2 || (T2 = 1));
  return F2;
}
var no = (t, e) => {
  let {
  ...
  ), O2 = W2(), z = O2.createBuffer(1, S2.length, O2.sampleRate);
  z.getChannelData(0).set(S2);
  const T2 = W2().createBufferSource();
  return T2.buffer = z, T2.start(e), {
    node: T2
  };
};
function go() {
...
var { Pattern: O, logger: W3, repl: $2 } = dist_exports;
so(W3);
...
function j2(e, {
  ...
  let i = 0;
  for (let l = h; l < p; l++) {
    const d = n[l] + 1, g = (a - f2 * (d - 1)) * c.height;
...
f.prototype.tscope = function(e = {}) {
  ...
  return this.analyze(t).draw(
    (o) => {
      var _a, _b;
      e.color = ((_b = (_a = o[0]) == null ? void 0 : _a.value) == null ? void 0 : _b.color) || W().foreground, e.color, C(e.smear, "0,0,0", e.ctx), j2(P[t], e);
    },
    { id: t }
  );
};
...
f.prototype.spectrum = function(e = {}) {
  ...
  return this.analyze(t).draw(
    (o) => {
      var _a, _b;
      e.color = ((_b = (_a = o[0]) == null ? void 0 : _a.value) == null ? void 0 : _b.color) || T[t] || W().foreground, T[t] = e.color, V(P[t], e);
    },
    { id: t }
  );
};
