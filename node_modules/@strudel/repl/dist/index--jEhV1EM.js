import { T as ne, U as q, V as g, W as K, X as re, Y as ie, Z as se } from "./index-DXN4EnIs.js";
class E {
  /**
   ...
   */
  hasListener(e, t) {
    return e === void 0 ? this.eventMap[E.ANY_EVENT] && this.eventMap[E.ANY_EVENT].length > 0 ? !0 : Object.entries(this.eventMap).some(([, n]) => n.length > 0) : this.eventMap[e] && this.eventMap[e].length > 0 ? t instanceof Y ? this.eventMap[e].filter((r) => r === t).length > 0 : typeof t == "function" ? this.eventMap[e].filter((r) => r.callback === t).length > 0 : t == null : !1;
  }
  /**
...
class A extends E {
  ...
  constructor(e) {
    ...
    for (let t = 1; t <= 16; t++) this.channels[t] = new Q(this, t);
   ...
   */
  send(e, t = { time: 0 }, n = 0) {
    if (e instanceof D && (e = o.isNode ? e.data : e.rawData), e instanceof Uint8Array && o.isNode && (e = Array.from(e)), l.validation) {
...
class O {
  ...
  forward(e) {
    this.suspended || this.types.includes(e.type) && (e.channel && !this.channels.includes(e.channel) || this.destinations.forEach((t) => {
      l.validation && !(t instanceof A) || t.send(e);
    }));
  }
}
/**
...
class X extends E {
  ...
  _dispatchParameterNumberEvent(e, t, n, r) {
    ...
    e === "rpn" ? s.parameter = Object.keys(i.REGISTERED_PARAMETERS).find((f) => i.REGISTERED_PARAMETERS[f][0] === t && i.REGISTERED_PARAMETERS[f][1] === n) : s.parameter = (t << 7) + n;
...
class Z extends E {
  ...
  addForwarder(e, t = {}) {
    let n;
    return e instanceof O ? n = e : n = new O(e, t), this._forwarders.push(n), n;
  }
  /**
  ...
  async _updateOutputs() {
    ...
    return this.interface.outputs.forEach((t) => {
      if (!this._outputs.find((n) => n._midiOutput === t)) {
        ...
        n || (n = new A(t)), this._outputs.push(n), e.push(n.open());
      }
    }), Promise.all(e);
  }
...
const oe = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  Utilities: o,
  WebMidi: l
}, Symbol.toStringTag, { value: "Module" })), { WebMidi: c } = oe;
function le() {
...
function B(a, e) {
  ...
  if (!r)
    throw new Error(
      `🔌 MIDI device '${r || ""}' not found. Use one of ${_(e)}`
    );
  return n ?? e[0];
}
...
function ee(a) {
  return Object.fromEntries(
    Object.entries(a).map(([e, t]) => (typeof t == "number" && (t = { ccn: t }), [K(e), t]))
  );
}
function de(a, e = "") {
...
let T = {};
async function ye(a) {
  typeof a == "string" && (a.startsWith("github:") && (a = de(a, "midimap.json")), T[a] || (T[a] = fetch(a).then((e) => e.json())), a = await T[a]), typeof a == "object" && Object.entries(a).forEach(([e, t]) => w.set(e, ee(t)));
}
const we = /* @__PURE__ */ new Map();
function ce(a = 0, e = 0, t = 1, n = 1) {
  if (e === t)
...
function he(a, e) {
  return Object.keys(e).filter((t) => !!a[K(t)]).map((t) => {
    const { ccn: n, min: r = 0, max: s = 1, exp: d = 1 } = a[t], h = ce(e[t], r, s, d);
    return { ccn: n, ccv: h };
  });
}
function v(a, e, t, n, r) {
...
function Ee(a, e, t, n, r, s) {
  ...
  const d = typeof a == "number" ? a : re(a), h = new b(d, { attack: e, duration: t });
  n.playNote(h, r, {
    time: s
  });
}
ne.prototype.midi = function(a, e = {}) {
  if (q(a))
    throw new Error(
  ...
  return J({
    onEnabled: ({ outputs: n }) => {
      const r = B(t.midiport, n), s = n.filter((d) => d.name !== r.name);
      g(
        `Midi enabled! Using "${r.name}". ${s?.length ? `Also available: ${_(s)}` : ""}`
      );
    },
    onDisconnected: ({ outputs: n }) => g(`Midi device disconnected! Available: ${_(n)}`)
  }), this.onTrigger((n, r, s, d) => {
    if (!c.enabled) {
      g("Midi not enabled");
      return;
    }
    n.ensureObjectValue();
    const h = t.latencyMs, f = `+${ie(d, r) + h}`;
    let {
      note: U,
      nrpnn: L,
      nrpv: P,
      ...
      sysexdata: z,
      midimap: S = t.midimap,
      midiport: j = t.midiport
    } = n.value;
    const u = B(j, c.outputs);
    if (!u) {
      g(
        `[midi] midiport "${j}" not found! available: ${c.outputs.map((N) => `'${N.name}'`).join(", ")}`
      );
      return;
    }
    if (I = te * I, w.has(S) ? he(w.get(S), n.value).forEach(({ ccn: C, ccv: R }) => v(C, R, u, p, f)) : S !== "default" && g(`[midi] midimap "${S}" not found! Available maps: ${[...w.keys()].join(", ")}`), U !== void 0 && !t.isController) {
      const N = n.duration.valueOf() / s * 1e3 - t.noteOffsetMs;
      Ee(U, I, N, u, p, f);
    }
    if (F !== void 0 && $(F, u, p, f), V !== void 0 && z !== void 0 && W(V, z, u, f), G !== void 0 && x !== void 0 && v(x, G, u, p, f), L !== void 0 && P !== void 0 && fe(L, P, u, p, f), H !== void 0 && ue(H, u, p, f), k !== void 0 && me(k, u, p, f), n.whole.begin + 0 === 0 && u.sendStart({ time: f }), ["clock", "midiClock"].includes(m))
      u.sendClock({ time: f });
      ...
      u.sendContinue({ time: f });
    else if (Array.isArray(m)) {
      if (m[0] === "progNum")
        $(m[1], u, p, f);
      else if (m[0] === "cc")
        m.length === 2 && v(m[0], m[1] / 127, u, p, f);
...
async function _e(a) {
  ...
  const e = await J(), t = B(a, c.inputs);
  if (!t)
    throw new Error(
      `midiin: device "${a}" not found.. connected devices: ${_(c.inputs)}`
    );
  if (e) {
    const r = c.inputs.filter((s) => s.name !== t.name);
    g(
      `Midi enabled! Using "${t.name}". ${r?.length ? `Also available: ${_(r)}` : ""}`
    );
  }
  y[a] || (y[a] = {});
  const n = (r) => se(() => y[a][r] || 0);
